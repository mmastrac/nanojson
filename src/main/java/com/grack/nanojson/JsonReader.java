...
public final class JsonReader {
	...
	public boolean next() throws JsonParserException {
		if (stateIndex == 0) {
			if (!first) {
				if (!tokener.eof)
					throw createTokenMismatchException(JsonTokener.TOKEN_EOF);
				return false;
			}
			token = tokener.advanceToToken();
		} else if (inObject) {
			int next = tokener.advanceToToken();
			if (next == JsonTokener.TOKEN_OBJECT_END) {
				first = false;
				inObject = states.get(--stateIndex);
				return false;
			}
		...
		if (token < JsonTokener.TOKEN_VALUE_MIN)
			throw createTokenMismatchException(JsonTokener.TOKEN_TRUE, JsonTokener.TOKEN_FALSE, JsonTokener.TOKEN_NULL,
					JsonTokener.TOKEN_NUMBER, JsonTokener.TOKEN_STRING, JsonTokener.TOKEN_ARRAY_START,
